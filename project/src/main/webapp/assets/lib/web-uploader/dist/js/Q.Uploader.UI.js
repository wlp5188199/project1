        ...
        draw: function (task) {
            ...
            var previewHtml = function(){
            	switch (name.substring(name.lastIndexOf('.')+1)){
            		...
            		case 'png':
            		case 'gif':
            			return '<img class="preview" src="' + URL.createObjectURL(task.file) + '" title="' + name + '"/>';
            		case 'mp4':
            			return '<video class="preview" src="' + URL.createObjectURL(task.file) + '" title="' + name + '" controls="controls"></video>';
            		default:
            			return '<div class="preview">暂不支持该类型文件预览</div>';
            	}
            }
